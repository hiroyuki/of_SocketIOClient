...
// Event handlers
void socketio_events::connected( const Value& args)
{
    AuEvents& event =AuEvents::GetInstance();
    IO_ConnectionEventData dt;
    dt.establised = true;
    ofNotifyEvent(event.connectionEvent, dt);
}

//{"name":"mode_changed","args":[{"mode":"swipe_single"}]}
void socketio_events::modeChanged(const Value& args)
{
    using namespace std;
    AuEvents& event =AuEvents::GetInstance();
    IO_ModeChangeEventData dt;
    cout << "modeCHanged:";;
    dt.mode = (&args[SizeType(0)].MemberBegin()->value)->GetString();
    ofNotifyEvent(event.modeEvent, dt);
}

//{"name":"session_started","args":[{"session_index":"40","started_at":1370447786743}]}
void socketio_events::sessionStarted(const Value& args)
{
    using namespace std;
    cout << "sessionStarted:";
    ...
    {
        AuEvents& event = AuEvents::GetInstance();
        IO_SessionStartEventData dt;
        for (Value::ConstMemberIterator itr = args[i].MemberBegin(); itr != args[i].MemberEnd(); ++itr)
        {
            if ( (string)itr->name.GetString() == "session_index")
            {
                dt.sessionIndex = itr->value.GetString();
            }
            else if ( (string)itr->name.GetString() == "started_at" )
            {
                long val = floor(itr->value.GetInt64() / 1000L);
                dt.startTime = val;
                dt.ms = itr->value.GetInt64() % 1000L;
            }
        }
        ofNotifyEvent(event.sessionEvent, dt);
    }
}

//{"name":"tapped","args":[{"user_token":"1234567890","count":3}]}
void socketio_events::tapped(const Value& args)
{
    using namespace std;
    cout << "tapped:";
    for (SizeType i = 0; i < args.Size(); i++)
    {
        ...
        for (Value::ConstMemberIterator itr = args[i].MemberBegin(); itr != args[i].MemberEnd(); ++itr)
        {
            if ( (string)itr->name.GetString() == "user_token")
            {
                dt.usertoken = itr->value.GetString();
            }
            else if ( (string)itr->name.GetString() == "count" )
            {
                dt.count = itr->value.IsInt64() ? itr->value.GetInt64() : itr->value.IsInt() ? itr->value.GetInt() : -1;
                if ( dt.count == -1) ofLogError() << "count is strange:" << itr->value.GetType();
            }
        }
        ofNotifyEvent(event.tapEvent, dt);
    }
}

//{"name":"swiped","args":[{"user_token":"1234567890","count":3}]}
void socketio_events::swiped(const Value& args)
{
    using namespace std;
    cout << "swiped:";
    for (SizeType i = 0; i < args.Size(); i++)
    {
        ...
        ofNotifyEvent(event.swipeEvent, dt);
    }
}

//{"name":"pa_registered","args":[{}]}
void socketio_events::paRegistered(const Value& args)
{
    using namespace std;
    for (SizeType i = 0; i < args.Size(); i++)
    {
        cout << "->paRegistered" << endl;
    }
}
...
void socketio_client_handler::emit(std::string name, std::vector<std::string> _args, std::string endpoint, unsigned int id) {
    Document d;
    d.SetObject();
